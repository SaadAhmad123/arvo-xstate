import { TelemetredSimpleMachineMemory, MachineMemoryRecord } from '../../src';
...
describe('TelemetredSimpleMachineMemory', () => {
  ...
  describe('write', () => {
    it('should throw error when writing with empty id', async () => {
      await expect(memory.write('', validData)).rejects.toThrow(
        'Machine ID is required for write operation',
      );
    });

    it('should throw error when writing undefined data', async () => {
      await expect(memory.write('test', undefined as any)).rejects.toThrow(
        'Data is required for write operation',
      );
    });

    it('should throw error when writing null data', async () => {
      await expect(memory.write('test', null as any)).rejects.toThrow(
        'Data is required for write operation',
      );
    });
  ...
  describe('lock', () => {
    it('should throw error when locking with empty id', async () => {
      await expect(memory.lock('')).rejects.toThrow(
  ...
  describe('unlock', () => {
    it('should throw error when unlocking with empty id', async () => {
      await expect(memory.unlock('')).rejects.toThrow(
