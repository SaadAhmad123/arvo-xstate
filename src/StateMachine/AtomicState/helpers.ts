  ...
  return context.with(trace.setSpan(context.active(), span), () => {
    ...
    try {
      return new AtomicState<TContext>({
        ...param,
        on: (param as Omit<IAtomicState<TContext>, 'always'> & {on: Record<string, Transition<TContext>>}).on,
        always: (param as Omit<IAtomicState<TContext>, 'on'> & {always: Transition<TContext>}).always,
      })
    } catch (error) {
